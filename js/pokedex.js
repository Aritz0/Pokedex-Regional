...
/* =========================
   ANIMACIÓN POKÉ BALL AL SELECCIONAR
========================= */
function playBallAnimation() {
    ...
    /* Quitar una animación anterior si seguía ahí */
    const old = screen.querySelector(".ball-anim");
    if (old) old.remove();

    /* Crear la Poké Ball + el destello */
    ...
    /* Colocar la animación centrada sobre el sprite */
    const place = () => {
        const sRect = screen.getBoundingClientRect();
        const iRect = imgEl.getBoundingClientRect();

        /* Centro del sprite relativo a la pantalla (sumando el scroll interno) */
        const cx = iRect.left - sRect.left + iRect.width / 2 + screen.scrollLeft;
        const cy = iRect.top - sRect.top + iRect.height / 2 + screen.scrollTop;

        wrap.style.left = cx + "px";
        wrap.style.top = cy + "px";
    };

    /* El sprite ya está en su sitio porque el detalle se mostró antes */
    place();
...
async function showPokemon(p) {
    ...
    /* SPRITE con respaldo + animación de aparición */
    handleBrokenImg(imgEl, p);
    delete imgEl.dataset.fallback;
    imgEl.src = getSprite(p);
    imgEl.classList.remove("appear");
    void imgEl.offsetWidth; /* reinicia la animación */
    imgEl.classList.add("appear");

    /* Animación de Poké Ball abriéndose justo sobre el sprite */
    playBallAnimation();
